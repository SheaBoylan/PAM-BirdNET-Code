# =============================================================================
# DISSERTATION: FULL ANALYSIS, FIGURES & TABLES
# Requires: all detection models and data objects from analysis script
# Run top to bottom to fit classification models, produce all figures and tables
#
# Classification models fitted here (betabinomial + random slope):
#   accuracy_int_bb_rs            — stand type best model
#   accuracy_activity_log_bb_rs   — masking best model
#
# Figures:
...
# Betabinomial family selected over binomial based on DHARMa diagnostics
# (ΔAIC > 220). Random slopes on distance by species added based on
# substantial AIC improvement (ΔAIC > 115) and species-level heterogeneity
# in accuracy (range 10.9%–92.6%). Persistent DHARMa deviations in final
# models attributed to irreducible species-level heterogeneity in BirdNET
# classification behaviour.
# =============================================================================

# Intermediate models retained for AIC table
...
# Overdispersion
check_overdispersion(stand_comparison_veg_int5)
check_overdispersion(veg_model_ba_LAI_int)
check_overdispersion(detection_mask_veg_log)
check_overdispersion(accuracy_int_bb_rs)
check_overdispersion(accuracy_activity_log_bb_rs)

# DHARMa residuals
simulateResiduals(stand_comparison_veg_int5,    plot = TRUE)
simulateResiduals(veg_model_ba_LAI_int,         plot = TRUE)
simulateResiduals(detection_mask_veg_log,       plot = TRUE)
simulateResiduals(accuracy_int_bb_rs,           plot = TRUE)
simulateResiduals(accuracy_activity_log_bb_rs,  plot = TRUE)

# Outlier investigation for detection_mask_veg_log (obs 282 flagged)
outliers(simulateResiduals(veg_model_ba_LAI_int))
outliers(simulateResiduals(detection_mask_veg_log))

# Multicollinearity (post-hoc; pre-modelling VIF check already confirmed
# no issues among raw predictors — elevated values here are artefacts of
# interaction terms)
check_collinearity(veg_model_ba_LAI_int)
check_collinearity(stand_comparison_veg_int5)

# Random effects likelihood ratio tests
anova(stand_comparison_veg_int5, update(stand_comparison_veg_int5, . ~ . - (1|scientific_name)))
anova(stand_comparison_veg_int5, update(stand_comparison_veg_int5, . ~ . - (1|site)))
anova(accuracy_int_bb_rs,        update(accuracy_int_bb_rs,        . ~ . - (1|transect_id)))
...
ci_veg           <- as.data.frame(confint(veg_model_ba_LAI_int))
ci_veg$term      <- rownames(ci_veg)
colnames(ci_veg) <- c("lwr", "upr", "estimate", "term")

coef_veg      <- as.data.frame(summary(veg_model_ba_LAI_int)$coefficients$cond)
...
pred_acc <- expand.grid(distance_m = dist_seq, stand_type = c("deciduous", "pine")) %>%
  ...
  predict_glmmTMB(model = accuracy_int_bb_rs) %>%
...
fig5 <- ggplot(species_plot, aes(x = accuracy_pct, y = expected_species, fill = tier)) +
  ...
  annotate("text", x = 93.5, y = Inf, label = "Desk-based baseline (93%)",
  ...
  scale_x_continuous(limits = c(0, 115), breaks = c(0, 25, 50, 75, 100),
                     ...
                     expand = expansion(add = c(0, 0))) +
  labs(x = "Classification accuracy (%)", y = NULL) +
  bar_theme +
  theme(legend.position = "bottom",
        legend.title    = element_text(face = "bold", size = 11),
        legend.text     = element_text(size = 11))

ggsave(file.path(out_dir, "Figure5_Species_Accuracy.png"), fig5, width = 8, height = 7, dpi = 300, bg = "white")
cat("✓ Figure 5 saved\n")
...
# =============================================================================
# FIGURE S2: Classification accuracy ~ Community vocal activity (null result)
# Model: accuracy_activity_log_bb_rs
# accuracy_int_bb_rs (without activity) preferred by AIC — included as
...
pred_acc_activity <- expand.grid(n_nontarget = seq(0, 344, length.out = 200),
  ...
  predict_glmmTMB(model = accuracy_activity_log_bb_rs)
...
# =============================================================================
# FIGURE S3: Random effects variance components
# Models: stand_comparison_veg_int5 (detection), accuracy_int_bb_rs (classification)
# =============================================================================

variance_data <- data.frame(
  model   = c("Detection", "Detection",
              "Classification", "Classification", "Classification"),
  group   = c("Species", "Site",
              "Species (intercept)", "Species (distance slope)", "Transect"),
  std_dev = c(1.34763, 0.67194,
              1.404019, 0.013276, 0.385447)
) %>%
  mutate(variance = std_dev^2,
         model    = factor(model, levels = c("Detection", "Classification")),
         group    = factor(group, levels = c("Species", "Site",
                                             "Species (intercept)", "Species (distance slope)", "Transect")))
...
figS3 <- ggplot(variance_data, aes(x = group, y = variance, fill = group)) +
  ...
  scale_fill_manual(values = c(Species = col_high, Site = col_pine,
                               "Species (intercept)"      = col_high,
                               "Species (distance slope)" = col_mid,
                               Transect                   = col_pine), guide = "none") +
...
aic_table <- bind_rows(
  ...
  make_aic_section("Classification: Stand Type (family selection)",
                   c("Binomial", "Betabinomial", "Betabinomial + Random slope (best)"),
                   accuracy_int, accuracy_int_bb, accuracy_int_bb_rs),
  
  make_aic_section("Classification: Masking (family selection)",
                   c("Binomial", "Betabinomial", "Betabinomial + Random slope (best)"),
                   accuracy_activity_log, accuracy_activity_log_bb, accuracy_activity_log_bb_rs)
)
...
fixed_effects <- bind_rows(
  ...
  extract_fixed(accuracy_int_bb_rs,           "Classification: Stand Type"),
  extract_fixed(accuracy_activity_log_bb_rs,  "Classification: Masking")
)
